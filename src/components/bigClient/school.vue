...
import {
  GetSchoolUserphysicals,
  OperateSchoolUserphysical,
...
export default {
  ...
  methods: {
    ...
    async modifySave() {
      ...
      try {
        return new Promise((resolve, reject) => {
          OperateSchoolUserphysical(allSelects)
            .then(res => {
              const loading = this.$loading({
                lock: true,
                text: "加载中",
      ...
      if (row) {
        ...
        this.$confirm("此操作将永久删除该文件, 是否继续？", "提示", {
          ...
          .then(() => {
            try {
              return new Promise((resolve, reject) => {
                OperateSchoolUserphysical(deleteData)
                  .then(res => {
                    const loading = this.$loading({
                      lock: true,
                      text: "加载中",
    ...
    async addOperation() {
      ...
      if (
        ...
        try {
          return new Promise((resolve, reject) => {
            debugger;
            OperateSchoolUserphysical(addInfo)
              .then(res => {
                const loading = this.$loading({
                  lock: true,
                  text: "加载中",
