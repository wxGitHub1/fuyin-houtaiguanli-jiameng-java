<template>
  ...
  <div>
    <el-row class="search">
      ...
      <el-col :span="2">
        <el-input v-model="seach.userName" style="width：100%" size="small" placeholder="请输入姓名"></el-input>
      </el-col>
      <el-col :span="2" style="width: 5.5%;line-height: 30px">
        <span>联系方式</span>
      </el-col>
      ...
      <el-col :span="2">
        <el-select clearable size="small" v-model="seach.status" placeholder="请选择">
          <el-option
      ...
      <el-col :span="1" style="width: 4.8%">
        <el-button size="small" @click="pageList(pages.currentPage,pages.pageSize)" type="warning" icon="el-icon-search">查询</el-button>
    ...
    <el-row class="client_info">
      ...
      <el-col :span="2">
        <el-select clearable size="small" v-model="seach.payType" placeholder="请选择">
          <el-option
      ...
      <el-col :span="2">
        <el-select clearable size="small" v-model="seach.owe" placeholder="请选择">
          <el-option
      ...
      <el-col :span="2">
        <el-input size="small" style="width：100%" v-model="seach.orderNum" placeholder="请输入订单编号"></el-input>
      </el-col>
      <el-col :span="2" style="width: 5.5%;line-height: 30px">
        <span>下单人</span>
      </el-col>
      <el-col :span="2">
        <el-input size="small" v-model="seach.createUserName" placeholder="请输入下单人姓名"></el-input>
      </el-col>
      <el-col :span="2" style="width: 5.5%;line-height: 30px">
        <span>病单类型</span>
      </el-col>
      ...
      <el-col :span="2">
        <el-select clearable size="small" v-model="seach.favorable" placeholder="请选择">
          <el-option
    ...
    <!-- table -->
    <el-table class="client_table" border :data="clientData" max-height="660">
    ...
    >
      ...
      <h3 class="b-b-p-1">
        ...
        <el-button
          type="primary"
          icon="el-icon-plus"
          @click="dialogMakeUpTheArrears = true"
          size="mini"
          class="right"
    ...
    >
      <el-table :data="refundData" border>
        ...
        <el-table-column prop="actual" label="实收价格" min-width="100"></el-table-column>
    ...
    >
      ...
      <!-- <div slot="footer" class="dialog-footer">
        <el-button @click="dialogSizeDetails=false" type="success">取消</el-button>
        <el-button type="primary" @click="addRefund()">确认退款</el-button>
      </div>-->
    </el-dialog>
    <!-- dialog 打印小票-->
    <el-dialog
          ...
          <div id="printTest">
            <div class="xiaopiao" v-if="dyxp.fw">
              ...
              <div class="clearfix">
                ...
                <span class="right font-b">{{receipt.lakala+receipt.cash+receipt.transfer}}</span>
              </div>
              <div>拉卡拉：{{receipt.lakala}}</div>
              <div>现金：{{receipt.cash}}</div>
              <div>转账：{{receipt.transfer}}</div>
              <hr class="dashed" />
              <div class="clearfix">
                <span class="left">下欠金额：</span>
                <span class="right font-b">{{receipt.oweMoney}}</span>
              </div>
              <hr />
              <div>订单编号：{{receipt.orderNum}}</div>
              <div>打印时间：{{realTime}}</div>
              <hr />
              <div>该产品不在医保、合疗、商业保险报销范围内，如有需求可提供正式增值税发票（抬头：陕西福音假肢有限责任公司）</div>
    ...
    >
      ...
      <el-table class="margin-t-10" :data="detailFormList" border max-height="500">
        ...
        <el-table-column prop="actual" label="实际价格">
          <template slot-scope="scope">
            <input class="input" type="text" v-model="scope.row.actual" oninput="value=value.replace(/[^\d]/g,'')"/>
          </template>
        </el-table-column>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="success" icon="el-icon-circle-check" @click="addRefund()">确认</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
import { exportMethod, getTime } from "../../utils/public";
...
export default {
  data() {
    return {
      ...
      imgUrl:
        javaApi.ExportUrl +
        "1b6101a5-0d6b-40df-9a85-e401c63e4283_1.jpg",
      clientData: [],
      seach: {
        userName: null,
        phone: null,
        ...
        owe: null,
        oweList: [{ name: "是", id: 1 }, { name: "否", id: "0" }],
        orderNum: null,
        createUserName: null,
        ...
        favorable: null,
        favorableList: [{ name: "有", id: 1 }, { name: "没有", id: "0" }]
      },
      //分页
      pages: {
        total: 10,
  ...
  methods: {
    ...
    orderingStart() {
      ...
      let data = {
        ...
        // price: this.paymentMethod.totalAmountReceivable,
        should: should,
    ...
    orderDetails(id) {
      ...
      productShape(data)
        .then(res => {
          console.log(res.data.data.size["取型"]);
          let list = [];
          let wc = res.data.data.size["取型"]["围长"];
          let kd = res.data.data.size["取型"]["宽度"];
          let zb = res.data.data.size["取型"]["足部"];
          let gd = res.data.data.size["取型"]["高度"];
          for (let i in wc) {
    ...
    details(orderNum) {
      ...
      orderDetail(data)
        .then(res => {
          ...
          this.imgUrl =
            javaApi.ExportUrl +
            res.data.data.orderDetailDto.url;
    ...
    exportExcels() {
      let data = {
        ...
        userName: this.seach.userName || null,
        phone: this.seach.phone || null,
        status: this.seach.status || null,
        payType: this.seach.payType || null,
        owe: this.seach.owe || null,
        orderNum: this.seach.orderNum || null,
        createUserName: this.seach.createUserName || null,
        prescriptionType: this.seach.prescriptionType || null,
        favorable: this.seach.favorable || null
      };
      const lsyObj = {
        method: "post",
        fileName: "订单信息",
